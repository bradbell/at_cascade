...
Predict Single Job with Failed Parent Job
#########################################
...
node.csv
********
For this example the root node, n0, has two children, n1 and n2. n2 has one child, n3
{xrst_code py}"""
csv_file['node.csv'] = \
...
1.0, 0.5, 1.5, 1.5 for nodes n0, n1, n2, n3 respectively.
{xrst_code py}"""
...
n3,male,50,2000,0.02,1.5
'''
"""{xrst_code}

fit_goal.csv
************
The goal is to fit the model for node n3.
{xrst_code py}"""
...
Note neither node n2 nor n3 have data defined, causing these nodes to
skip fitting. This ensures that predicting n3 will require an ancestor
fit above n2.
{xrst_code py}"""
...
def main() :
   ...
   #
   # node2haqi, haqi_avg
   node2haqi  = { 'n0' : 1.0, 'n1' : 0.5, 'n2' : 1.5, 'n3' : 1.5}
   ...
   # csv.predict non-root node
   at_cascade.csv.predict(fit_dir,start_job_name="n3.female")
   #
   # prefix
   for prefix in [ 'fit' , 'sam' ] :
      #
      # predict_table
      file_name = f'{fit_dir}/predict/{prefix}_n3.female.csv'
      predict_table = at_cascade.csv.read_table(file_name)
      #
      # check node_name and fit_node_name
      assert predict_table[1][ 'node_name' ] == 'n3'
      assert predict_table[1][ 'fit_node_name' ] == 'n0'

if __name__ == '__main__' :
   main()
   print('ancestor_fail.py: OK')
# END_PROGRAM
